...
try:
   from fixation import initialise_population_on_NC
...
def run_simulation_until_Tmax_and_save(N, mu, Tmax, no_runs, s1, s2, p0, p1, p2, str_startgene, string_for_saving_plot, factor_initialisation, ncpus=15):
   global phenotypes
   ...
   fixation = [] #fixation results: enter 0 for no fixation, 1 for p1 fixation and 2 for p2
   fixation_time = []
   p1_found = np.zeros(no_runs, dtype='int16')
   p2_found = np.zeros(no_runs, dtype='int16')
   ########evolution on this map  
   fixation_function_with_param = partial(run_simulation_until_Tmax, N=N, mu=mu, Tmax=Tmax, s1=s1, s2=s2, p0=p0, p1=p1, p2=p2, factor_initialisation=factor_initialisation) #, phenotypes=phenotypes)
   ...
   for run in range(no_runs):
       fixation_thisrun, p1_found_thisrun, p2_found_thisrun, discoverytime_p1, discoverytime_p2, fixation_times_thisrun = result_list[run] 
       print('p1_found_thisrun, p2_found_thisrun, fixation_thisrun', p1_found_thisrun, p2_found_thisrun, fixation_thisrun)
       fixation.append('_'.join([str(f) for f in fixation_thisrun]))
       fixation_time.append('_'.join([str(f) for f in fixation_times_thisrun]))
   ...
   with open('./data/longrun_biomorphs_fixation_'+string_pop_param + string_for_saving_plot+'.txt', 'w') as f:
      for s in fixation:
         f.write(str(s) + '\n')
   with open('./data/longrun_biomorphs_fixation_time_'+string_pop_param + string_for_saving_plot+'.txt', 'w') as f:
      for s in fixation_time:
         f.write(str(s) + '\n')
   np.save( './data/longrun_biomorphs_p1found_'+string_pop_param + string_for_saving_plot+'.npy', p1_found)
   np.save( './data/longrun_biomorphs_p2found_'+string_pop_param + string_for_saving_plot+'.npy', p2_found)
   np.save( './data/longrun_biomorphs_discoverytimes_'+string_pop_param + string_for_saving_plot+'.npy', discovery_times_p1_p2)
   

def run_simulation_until_Tmax(filename_for_initial_condition, N, mu, Tmax, s1, s2, p0, p1, p2, factor_initialisation): #, phenotypes):
   ...
   starttime = time.time()
   ##
   p1_found_thisrun, p2_found_thisrun, discoverytime_p1, discoverytime_p2, fixation_thisrun, fixation_times = 0, 0, 0, 0, [0,], [0,]
   ...
   for t in range(0, int(Tmax)):
      ...
      number_p1_in_pop, number_p2_in_pop, number_p0_in_pop = (P_p_current == p1).sum(), (P_p_current == p2).sum(), (P_p_current == p0).sum()
      ...
      if number_p1_in_pop > 0.7*N and fixation_thisrun[-1] != 1:
         fixation_thisrun.append(1)
         fixation_times.append(t)
         print( 'fixation of p1')
      elif number_p2_in_pop > 0.7*N and fixation_thisrun[-1] != 2:
         fixation_thisrun.append(2)
         fixation_times.append(t)
         print( 'fixation of p2')
      elif number_p0_in_pop > 0.7*N and fixation_thisrun[-1] != 0:
         fixation_thisrun.append(0)
         fixation_times.append(t)
         print( 'fixation of p0')
      if t%10**4==0 and t!=0:
         print( str(float(t)/Tmax*100)+'% finished', flush=True)
   ...
   print('number of fixations', len(fixation_thisrun), 'after time', t)
   return fixation_thisrun, p1_found_thisrun, p2_found_thisrun, discoverytime_p1, discoverytime_p2, fixation_times